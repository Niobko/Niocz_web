...
test("BOMBANANA is registered consistently across the site", () => {
  ...
  assert.equal(game.supportedVersion, "v1.0.2");
  assert.equal(game.verifiedBuildId, "25239138");
  assert.equal(game.currentBuildId, "25239138");
...
test("detail uses the requested SEO, Czech content and shared feature hooks", () => {
  ...
  assert.match(detail, /Bombanaa_NioCZ\.zip/);
  assert.match(detail, /0\.18 MB/);
  assert.match(detail, /3\. 9\. 2026/);
  assert.match(detail, /12\. 9\. 2026/);
  ...
  assert.match(detail, /Spusťte instalátor \(<code>install\.exe<\/code>\)/);
  assert.match(detail, /přepněte jazyk na <code>Čeština<\/code>/);
  assert.match(detail, /Ověřit integritu herních souborů/);
  assert.match(detail, /zvolte <strong>Vlastnosti \/ Správa<\/strong>/);
