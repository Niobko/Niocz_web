-- NioCZ LOC: Vacation Cafe Simulator
...
alter table public.comments
  drop constraint if exists comments_game_slug_check_v10;

alter table public.comments
  add constraint comments_game_slug_check_v10
  check (game_slug in (
    'alchemy-factory',
    'bookshop-simulator',
    'bombanana',
...
alter table public.comments
  validate constraint comments_game_slug_check_v10;

alter table public.comments
  drop constraint if exists comments_game_slug_check;

alter table public.comments
  rename constraint comments_game_slug_check_v10
  to comments_game_slug_check;

alter table public.bug_reports
  drop constraint if exists bug_reports_game_slug_check_v10;

alter table public.bug_reports
  add constraint bug_reports_game_slug_check_v10
  check (game_slug in (
    'alchemy-factory',
    'bookshop-simulator',
    'bombanana',
...
alter table public.bug_reports
  validate constraint bug_reports_game_slug_check_v10;

alter table public.bug_reports
  drop constraint if exists bug_reports_game_slug_check;

alter table public.bug_reports
  rename constraint bug_reports_game_slug_check_v10
  to bug_reports_game_slug_check;

alter table public.game_ratings
  drop constraint if exists game_ratings_game_slug_check_v10;

alter table public.game_ratings
  add constraint game_ratings_game_slug_check_v10
...
alter table public.game_ratings
  validate constraint game_ratings_game_slug_check_v10;

alter table public.game_ratings
  drop constraint if exists game_ratings_game_slug_check;

alter table public.game_ratings
  rename constraint game_ratings_game_slug_check_v10
  to game_ratings_game_slug_check;

alter table public.download_totals
  drop constraint if exists download_totals_game_slug_check_v10;

alter table public.download_totals
  add constraint download_totals_game_slug_check_v10
  check (game_slug in (
    'alchemy-factory',
    'bookshop-simulator',
    'bombanana',
