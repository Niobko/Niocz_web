...
alter table public.comments
  drop constraint if exists comments_game_slug_check_v18;

alter table public.comments
  add constraint comments_game_slug_check_v18
  check (game_slug in (
    ...
    'kynseed', 'leafy-corner', 'no-mans-sky', 'scrap-mechanic', 'parcel-simulator',
...
alter table public.comments
  validate constraint comments_game_slug_check_v18;

alter table public.comments
  drop constraint if exists comments_game_slug_check;

alter table public.comments
  rename constraint comments_game_slug_check_v18
  to comments_game_slug_check;

alter table public.bug_reports
  drop constraint if exists bug_reports_game_slug_check_v18;

alter table public.bug_reports
  add constraint bug_reports_game_slug_check_v18
  check (game_slug in (
    'alchemy-factory', 'arms-of-god', 'astroneer', 'ready-or-not',
    'shapez-2', 'prince-of-persia-the-lost-crown', 'bookshop-simulator',
    'bombanana', 'breathedge-2', 'catmailco', 'cloverpit', 'e-shop-tycoon',
...
alter table public.bug_reports
  validate constraint bug_reports_game_slug_check_v18;

alter table public.bug_reports
  drop constraint if exists bug_reports_game_slug_check;

alter table public.bug_reports
  rename constraint bug_reports_game_slug_check_v18
  to bug_reports_game_slug_check;

alter table public.game_ratings
  drop constraint if exists game_ratings_game_slug_check_v18;

alter table public.game_ratings
  add constraint game_ratings_game_slug_check_v18
...
alter table public.game_ratings
  validate constraint game_ratings_game_slug_check_v18;

alter table public.game_ratings
  drop constraint if exists game_ratings_game_slug_check;

alter table public.game_ratings
  rename constraint game_ratings_game_slug_check_v18
  to game_ratings_game_slug_check;

alter table public.download_totals
  drop constraint if exists download_totals_game_slug_check_v18;

alter table public.download_totals
  add constraint download_totals_game_slug_check_v18
  check (game_slug in (
    'alchemy-factory', 'arms-of-god', 'astroneer', 'ready-or-not',
    'shapez-2', 'prince-of-persia-the-lost-crown', 'bookshop-simulator',
    'bombanana', 'breathedge-2', 'catmailco', 'cloverpit', 'e-shop-tycoon',
